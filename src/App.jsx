...
// Também salva a importação ANTERIOR para permitir comparação.
...
async function salvarFatRowsAtual(rows, nome) {
  try {
    const db = await abrirSlaDB();
    // Antes de salvar o atual, promove o atual para "anterior"
    const atual = await carregarFatRowsSalvo();
    if (atual) {
      await new Promise((resolve, reject) => {
        const tx = db.transaction(SLA_STORE, "readwrite");
        tx.objectStore(SLA_STORE).put(atual, "coletaRecebimentoAnterior");
        tx.oncomplete = resolve;
        tx.onerror = () => reject(tx.error);
      });
    }
...
async function carregarFatRowsAnterior() {
  try {
    const db = await abrirSlaDB();
    return await new Promise((resolve, reject) => {
      const tx = db.transaction(SLA_STORE, "readonly");
      const req = tx.objectStore(SLA_STORE).get("coletaRecebimentoAnterior");
...
const AbaFaturamentoColeta = ({
  ...
  const [rowsAnt, setRowsAnt] = useState([]);
  const [nomeAnt, setNomeAnt] = useState("");

  // Carrega o snapshot anterior ao montar
  useEffect(() => {
    carregarFatRowsAnterior().then(salvo => {
      if (salvo?.rows) { setRowsAnt(salvo.rows); setNomeAnt(salvo.nome||""); }
    });
  }, []);
  ...
  const carregar = (file) => {
    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        ...
        setNome(file.name);
        setLoading("");
        salvarFatRowsAtual(results.data, file.name).then(ok => {
          setAvisoPersistencia(!ok);
          // Após salvar, recarrega o anterior (que acabou de ser promovido)
          carregarFatRowsAnterior().then(salvo => {
            if (salvo?.rows) { setRowsAnt(salvo.rows); setNomeAnt(salvo.nome||""); }
          });
        });
  ...
  // Análise do arquivo anterior (sem filtros de período/parceiro — visão geral)
  const analiseAnt = useMemo(() => {
    if (!rowsAnt.length) return null;
    const validas = rowsAnt
      .filter(r => r["Data Coleta"] && r["Data de Recebimento da Coleta"])
      .map(r => ({...r, dias: calcularDias(r["Data Coleta"], r["Data de Recebimento da Coleta"])}))
      .filter(r => r.dias !== null && r.dias >= 0);
    const n = validas.length;
    ...
    return {n, mesma, um, dois, tres, porParceiro};
  }, [rowsAnt, calcularDias]);

  const pct = (v,n) => n>0 ? `${(v/n*100).toFixed(2)}%` : "—";
  const pctNum = (v,n) => n>0 ? v/n*100 : null;
  const fmtDelta = (atualV, atualN, antV, antN) => {
    const pA = pctNum(atualV, atualN);
    const pAnt = pctNum(antV, antN);
    if (pA == null || pAnt == null) return null;
    const d = Math.round((pA - pAnt) * 100) / 100;
    return d;
  };
    ...
    <div style={{marginTop:28}}>
      ...
      {rows.length>0 && <>
        ...
        <div style={{fontSize:12, color:C.cinzaTexto, marginBottom:10}}>
          ...
          {analiseAnt && nomeAnt && <span style={{marginLeft:12, background:"#FEF3C7", border:"1px solid #FBBF24", borderRadius:6, padding:"2px 8px", color:"#92400E"}}>
            Comparando com: <strong>{nomeAnt}</strong> ({analiseAnt.n} coletas)
          </span>}
        </div>
        ...
        <table style={{width:"100%", borderCollapse:"collapse", fontSize:13, marginBottom:22}}>
          <thead>
            <tr style={{background:C.cinzaFundo}}>
              ...
              {analiseAnt&&<th style={{textAlign:"right", padding:"8px 12px", color:C.cinzaTexto}}>% Anterior</th>}
              {analiseAnt&&<th style={{textAlign:"right", padding:"8px 12px", color:C.cinzaTexto}}>Δ</th>}
            </tr>
          </thead>
          <tbody>
            {[
              {label:"Mesma data",            vA:analise.mesma, vAnt:analiseAnt?.mesma},
              {label:`Exatamente 1 ${unidade}`,vA:analise.um,   vAnt:analiseAnt?.um},
              {label:`Exatamente 2 ${unidadePl}`,vA:analise.dois,vAnt:analiseAnt?.dois},
              {label:`3 ${unidadePl} ou mais`,  vA:analise.tres, vAnt:analiseAnt?.tres},
            ].map(({label,vA,vAnt},i)=>{
              const d = analiseAnt ? fmtDelta(vA,analise.n,vAnt,analiseAnt.n) : null;
              const corD = d==null?null:d>=0?C.verde:C.vermelho;
              return (
                <tr key={i} style={{borderTop:`1px solid ${C.cinzaBorda}`}}>
                  <td style={{padding:"8px 12px"}}>{label}</td>
                  <td style={{textAlign:"right",padding:"8px 12px"}}>{vA}</td>
                  <td style={{textAlign:"right",padding:"8px 12px",fontWeight:600}}>{pct(vA,analise.n)}</td>
                  {analiseAnt&&<td style={{textAlign:"right",padding:"8px 12px",color:C.cinzaTexto}}>{pct(vAnt,analiseAnt.n)}</td>}
                  {analiseAnt&&<td style={{textAlign:"right",padding:"8px 12px",fontWeight:700,color:corD}}>
                    {d==null?"—":`${d>=0?"+":""}${d.toFixed(2)} p.p.`}
                  </td>}
                </tr>
              );
            })}
            <tr style={{fontWeight:700, borderTop:`2px solid ${C.cinzaBorda}`}}>
              ...
              <td style={{textAlign:"right",padding:"8px 12px"}}>{analise.n}</td>
              <td style={{textAlign:"right",padding:"8px 12px"}}>100%</td>
              {analiseAnt&&<td style={{textAlign:"right",padding:"8px 12px",color:C.cinzaTexto}}>100%</td>}
              {analiseAnt&&<td style={{padding:"8px 12px"}}/>}
            </tr>
          </tbody>
        </table>
        ...
        {parceiroSel==="Todos" && Object.keys(analise.porParceiro).length>1 && <>
          <div style={{fontWeight:700, fontSize:14, marginBottom:8}}>
            Por parça
            {analiseAnt && nomeAnt && <span style={{fontSize:11, fontWeight:400, color:C.cinzaTexto, marginLeft:8}}>Δ vs {nomeAnt}</span>}
          </div>
          <table style={{width:"100%", borderCollapse:"collapse", fontSize:12.5}}>
            <thead>
              <tr style={{background:C.cinzaFundo}}>
                ...
                {analiseAnt&&<th style={{textAlign:"right", padding:"6px 10px", color:C.cinzaTexto}}>Δ</th>}
            ...
            <tbody>
              {Object.entries(analise.porParceiro)
                ...
                .map(([p,d])=>{
                  const dAnt = analiseAnt ? (analiseAnt.porParceiro[p]||null) : null;
                  const dd0 = fmtDelta(d.mesma,d.total,dAnt?dAnt.mesma:0,dAnt?dAnt.total:0);
                  const dd1 = fmtDelta(d.um,d.total,dAnt?dAnt.um:0,dAnt?dAnt.total:0);
                  const dd2 = fmtDelta(d.dois,d.total,dAnt?dAnt.dois:0,dAnt?dAnt.total:0);
                  const dd3 = fmtDelta(d.tres,d.total,dAnt?dAnt.tres:0,dAnt?dAnt.total:0);
                  const cor0=dd0==null?null:dd0>=0?C.verde:C.vermelho;
                  const cor1=dd1==null?null:dd1>=0?C.verde:C.vermelho;
                  const cor2=dd2==null?null:dd2>=0?C.verde:C.vermelho;
                  const cor3=dd3==null?null:dd3>=0?C.verde:C.vermelho;
                  const fmtD=(dd,cor)=>dd==null?"—":`${dd>=0?"+":" "}${dd.toFixed(2)}`;
                  return (
                  <tr key={p}>
                    ...
                    {analiseAnt&&<td style={{textAlign:"right",padding:"6px 10px",fontWeight:600,color:cor0,fontSize:11}}>{fmtD(dd0,cor0)}</td>}
                    <td style={{textAlign:"right",padding:"6px 10px"}}>{pct(d.um,d.total)}</td>
                    {analiseAnt&&<td style={{textAlign:"right",padding:"6px 10px",fontWeight:600,color:cor1,fontSize:11}}>{fmtD(dd1,cor1)}</td>}
                    <td style={{textAlign:"right",padding:"6px 10px"}}>{pct(d.dois,d.total)}</td>
                    {analiseAnt&&<td style={{textAlign:"right",padding:"6px 10px",fontWeight:600,color:cor2,fontSize:11}}>{fmtD(dd2,cor2)}</td>}
                    <td style={{textAlign:"right",padding:"6px 10px"}}>{pct(d.tres,d.total)}</td>
                    {analiseAnt&&<td style={{textAlign:"right",padding:"6px 10px",fontWeight:600,color:cor3,fontSize:11}}>{fmtD(dd3,cor3)}</td>}
                  </tr>
                );})}
            </tbody>
          </table>
...
export default function SlaApp() {
  ...
  const getRaw = useCallback((p,periodo)=>{
    ...
    if(granular==="mes") return getRawMes(p,periodo);
    if(granular==="trim"){
      const meses=TRIM_MESES[periodo]||[];
      const results=meses.map(m=>getRawMes(p,m)).filter(Boolean);
  ...
  const variacaoParceiros = useMemo(()=>{
    let periodos=[];
    if(granular==="semana"&&semanasSel.length>0){
      const atual=Math.max(...semanasSel); // semana mais recente marcada no filtro
      const idx=ALL_SEMANAS.indexOf(atual);
      const ant=idx>0?ALL_SEMANAS[idx-1]:null;
      if(ant!=null) periodos=[ant,atual];
    } else if(granular==="mes"&&mesesSel.length>0){
      const atual=Math.max(...mesesSel); // mês mais recente marcado no filtro
      const idx=ALL_MESES.indexOf(atual);
      const ant=idx>0?ALL_MESES[idx-1]:null;
      if(ant!=null) periodos=[ant,atual];
    }
