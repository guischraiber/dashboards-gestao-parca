          ...
          <div className="card">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 18, flexWrap: 'wrap' }}>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 12, color: '#6B7280' }}>Taxa atual</div>
                <div style={{ fontSize: 26, fontWeight: 700 }}>{atual.impactoTaxa.taxaBase.toFixed(2)}%</div>
              </div>
              <div style={{ fontSize: 22, color: '#9CA3AF' }}>→</div>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 12, color: '#6B7280' }}>Nova taxa (próximos 3 meses)</div>
                <div style={{ fontSize: 26, fontWeight: 700, color: CORES[atual.faixa.cor] }}>
                  {atual.impactoTaxa.taxaNova.toFixed(2)}%
                </div>
              </div>
              <span className={`badge ${atual.faixa.cor}`} style={{ marginLeft: 8 }}>
                {atual.impactoTaxa.ajustePP > 0 ? '+' : ''}{atual.impactoTaxa.ajustePP.toFixed(2)} p.p.
              </span>
            </div>
            <p style={{ fontSize: 13, color: '#6B7280', marginTop: 12, marginBottom: 0 }}>
              Essa taxa passa a valer em coleta reversa, devolução Bulky e terceiras nos próximos 3 meses,
              conforme o resultado do seu score neste trimestre.
