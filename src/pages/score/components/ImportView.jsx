...
export default function ImportView({ token }) {
  ...
  async function handleEnviarModelo() {
    ...
    enviar({ admin: token, modeloCsv }, setEnviandoModelo, setResultadoModelo, () => setModelo(null));
  }

  async function handleEnviarAvancado() {
    const body = { admin: token };
    if (indicadores) body.indicadoresCsv = await lerArquivoComoTexto(indicadores);
    if (faturamento) body.faturamentoCsv = await lerArquivoComoTexto(faturamento);
    if (taxas) body.taxasCsv = await lerArquivoComoTexto(taxas);
